 ...
 * @desc    Belirli bir sahaya ait rezervasyonlarÄ± Ã§eker
 */
router.get('/facility/:pitchId', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(
            \SELECT
                b.id, b.start_time as " rezDate\, EXTRACT(HOUR FROM b.start_time)::int as \rezHour\,
 b.total_price, b.status, b.payment_method, b.notes as \note\,
 ...
 console.error('Rezervasyonlari Getirme Hatasi:', error);
 res.status(500).json({ success: false, error: error.message });
 }
});
